# Awesome Digital Nomad 🌍

> **The Ultimate Guide to Geo-Arbitrage, Solo Business, and Freedom.**
> *Build your global identity, automate your income, and live anywhere.*

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Twitter Follow](https://img.shields.io/twitter/follow/MeiJynX?style=social)](https://twitter.com/MeiJynX)

---

## 🚀 What is this?

This repository is the open-source home of **"Digital Nomad Mastery"** (Project Name: `awesome-digital-nomad`). It documents the complete journey of becoming a full-stack digital nomad in 2025.

Unlike generic travel blogs, this is a **technical & strategic roadmap** for building a **One-Person Business** that leverages global arbitrage.

### 📚 Core Content Structure

This repo is organized into clear modules:

*   **[`/book`](./book/)**: The complete open-source book content.
    *   [Outline & Syllabus](./book/README.md)
    *   [Chapter 1: The Stability Paradox](./book/01-mindset/chapter-01-stability-paradox-v2.md)
*   **[`/resources`](./resources/)**: Actionable tools and templates.
    *   [Social Media Setup SOP](./resources/sops/social-media-setup.md)
    *   [Content Repurposing Matrix](./resources/templates/content-matrix.md)
*   **[`/planning`](./planning/)**: Strategic roadmaps and project plans.
    *   [Course Curriculum v4.0](./planning/PROJECT_PLAN_v4_Course_Curriculum.md)

---

## 🌟 Features

*   **Geo-Arbitrage**: Earn in USD/EUR, spend in THB/IDR.
*   **Solo Founder Stack**: Tools for running a business of one (AI, No-Code, Automation).
*   **Global Infrastructure**: Banking (Wise), Visas (DTV), and Connectivity (eSIM).
*   **Anti-Fragile**: Building income streams that don't depend on a single employer.

---

## 🤝 Connect with MeiJynX

I document my journey and share raw data/insights across these platforms. **Follow to stay updated:**

### 🐦 Socials
*   **Twitter / X**: [@MeiJynX](https://twitter.com/MeiJynX) (Daily thoughts & threads)
*   **YouTube**: [@MeiJynX](https://youtube.com/@MeiJynX) (Deep dive tutorials)
*   **LinkedIn**: [MeiJynX](https://linkedin.com/in/MeiJynX) (Professional network)
*   **Instagram**: [@MeiJynX](https://instagram.com/MeiJynX) (Lifestyle & Behind the scenes)

### 🛠️ Maker Platforms
*   **GitHub**: [MeiJynX](https://github.com/MeiJynX) (Code & Open Source)
*   **Product Hunt**: [@MeiJynX](https://www.producthunt.com/@MeiJynX) (Product launches)
*   **Indie Hackers**: [MeiJynX](https://www.indiehackers.com/MeiJynX) (Revenue & Growth)

### 💰 Support & Products
*   **Gumroad**: [MeiJynX.gumroad.com](https://MeiJynX.gumroad.com) (Get my eBooks & Templates)
*   **Buy Me a Coffee**: [buymeacoffee.com/MeiJynX](https://www.buymeacoffee.com/MeiJynX) (Fuel my caffeine addiction)

---

## 🗺️ Roadmap

- [x] **Phase 1: Foundation** (Identity, Banking, VPN)
- [x] **Phase 2: Strategy** (Book Outline, Content Matrix)
- [ ] **Phase 3: Production** (Drafting Chapters 2-15)
- [ ] **Phase 4: Launch** (Publishing to Gumroad/Amazon)

---

## ❤️ Contributing

This project is open source because I believe knowledge should be free.
If you find a typo, have a better tool suggestion, or want to translate this into another language:

1.  Fork this repo.
2.  Create a branch (`git checkout -b fix/typo`).
3.  Commit your changes.
4.  Open a Pull Request.

---

*Built with ❤️ by [MeiJynX](https://twitter.com/MeiJynX) from somewhere on Earth.*
